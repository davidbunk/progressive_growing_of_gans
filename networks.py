...
#----------------------------------------------------------------------------
# Discriminator network used in the paper for phase 1.
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    if structure == 'recursive':
        def grow(res, lod):
            ...
            if lod > 0: x = cset(x, ((lod_in - 2) < lod), lambda: grow(res + 1, lod - 1))
            x = block(x(), res); y = lambda: x
            if res > 2: y = cset(y, ((lod_in - 2) > lod), lambda: lerp(x, fromrgb(downscale2d(images_in, 2**(lod+1)), res - 1), (lod_in - 2) - lod))
            return y()
...
#----------------------------------------------------------------------------
# Discriminator network used in the paper phase 2.
